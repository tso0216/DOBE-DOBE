"""v2_dae_fsce：v2_dae（denoising）再加上 FSCE（fuzzy set cross entropy，UMAP
訓練用的那個 loss），等於「輸入端加噪」跟「latent 端對齊高維鄰接關係」
兩個手段同時開，encoder 出口一樣不接激活（沿用 v2_ae/v2_dae 的設計）。

為什麼要有這一版：v2_dae 用加噪逼 encoder 從殘缺觀測去猜完整分布，希望
latent 抓的是「類別組成」而不是「總量」；但 Poisson NLL 本身並不管 latent
的幾何長什麼樣，組成相近的 patch 不保證會被放在一起。FSCE 直接把高維
kNN 的模糊隸屬度當監督訊號拉進 latent 距離，剛好補上這一塊。兩者的目標
其實是同一個方向——都在削弱「總量 = latent」這條捷徑——所以預期是相加而
不是打架；要看的是加噪之後高維鄰接圖本身還算不算數（圖是用乾淨資料建的，
但 encoder 訓練時看的是加噪版）。

FSCE 的高維鄰接關係是用「整包 count 向量的 log1p、cosine 距離」kNN 建的
fuzzy simplicial set（跟 data/patch/umap_grid.py 的可行性檢查用同一種 metric），
用乾淨 count 建、只建一次、整個訓練共用同一張圖，不是每個 epoch 重建。
...
def corrupt(x, p, mode="thinning", generator=None):
    ...
    p：破壞強度 ∈[0,1)。thinning 是每個 POI 被丟掉的機率，mask 是每一類
       整維被抹成 0 的機率。p=0 時直接回傳 x（等價於退化成 v2_ae_fsce）。
...
class MLPAE(nn.Module):
    """跟 v2_dae 的 MLPAE 完全同架構（encoder 出口不接激活），
    denoising 一樣完全發生在資料端（見 corrupt()），模型本身不需要知道。
    """
