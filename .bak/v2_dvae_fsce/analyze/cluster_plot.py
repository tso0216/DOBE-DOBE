"""看 v2_dvae_fsce（Dirichlet 先驗 + FSCE 拓撲約束）把 patch 分成了什麼。
...
def main():
    ...
    fig.suptitle(f"{VERSION} Dirichlet-VAE + FSCE 分群（{len(theta)} 個 patch，"
                 f"K={k}（自由度 2，對齊 v2_vae 的 latent_dim=2），"
                 f"alpha={alpha:.3f}，Poisson NLL + BETA·Dirichlet KL + LAMBDA·FSCE）",
                 fontsize=12)
    fig.tight_layout()
