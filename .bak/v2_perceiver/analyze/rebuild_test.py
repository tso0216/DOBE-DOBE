"""挑一個 patch 丟進訓練好的 v2_perceiver AE，比較「進去」與「出來」的類別 count。

用 --n 指定 patch 編號（0 起算）。跟 v2_ae 的 rebuild_test.py 是同一種圖
（長條圖比真實 count 與重建 λ），差別只在 encoder 吃的是原始 token
集合而不是先算好的聚合向量。
"""
...
from ae import PerceiverAE, N_CAT, Patches, poisson_deviance, poisson_nll  # noqa: E402
...
def main():
    ...
    ax.bar(idx_c - w / 2, cnt, width=w, color=CAT_COLORS, alpha=0.55,
           label="真實 count")
    ax.bar(idx_c + w / 2, lam, width=w, color=CAT_COLORS, alpha=0.95,
