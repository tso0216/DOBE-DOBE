"""訓練 v2_vae_fsce：跟 v2_vae 的 train.py 相比，多了兩件事——訓練前先在高維
count 向量上建一次 FSCE 用的 fuzzy graph，訓練迴圈裡每個 step 除了原本的
reconstruction + KL batch，額外抽一批正樣本邊（來自 fuzzy graph）跟等量的
隨機負樣本邊，對 mu 算 FSCE loss，加權後跟 (Poisson NLL + BETA*KL) 加在
一起、共用同一次 backward/opt.step()。細節同 v2_vae_tanh_fsce，差別只在
底下的 VAE 沒有 tanh。

FSCE 權重用線性 warm-up：前 FSCE_WARMUP_EPOCHS 個 epoch 是 0，之後花同樣長度
線性升到 LAMBDA_FSCE，先讓 encoder 學出有意義的 count -> mu 映射，再逐步加
拓撲約束，避免兩個 loss 一開始就互相打架。
"""
