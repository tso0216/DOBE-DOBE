"""訓練 v3_ddae_gat：v2_ddae_fsce 加一條幾何分支（GAT），loss 完全沒動。

encoder 從單一 MLP 變成雙分支——count 分支是原本那 4 層、幾何分支吃 patch 內
POI 的座標距離矩陣，兩支各出 HIDDEN 維、concat 之後用同一個 head 壓成 z。
decoder、Poisson NLL、FSCE、所有超參數都跟 v2_ddae_fsce 一樣，所以 val_dev
與 expl_dev 是同一把尺，可以直接比「加了幾何有沒有讓重建變好」。
