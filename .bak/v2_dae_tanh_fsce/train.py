"""訓練 v2_dae_tanh_fsce：denoising + tanh + FSCE 三個變因全開的版本。

跟 v2_dae_fsce 的 train.py 完全一樣，只是換了 VERSION/模型（encoder 多一個
Tanh()）。所有餵進 encoder 的輸入（reconstruction batch 跟 FSCE 的 pair
兩邊都算）都先過 corrupt()，Poisson NLL 的目標仍然是乾淨的原始 count；
fuzzy graph 用乾淨 count 建（鄰接關係是資料的性質，不是噪聲的性質）。

驗證與最後輸出 latent 的推論階段一律不加噪。破壞是每個 step 重新抽的
（generator 綁 SEED+epoch，可重現）。

超參數全部跟 v2_dae_fsce / v2_ae_tanh_fsce 對齊，這樣跟它們比較時，
差異只來自各自缺的那一個變因。
"""
