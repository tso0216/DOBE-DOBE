"""v2_dae_tanh_fsce：三個變因全開——denoising（輸入端加噪）、tanh（latent 被
夾在 (-1,1)）、FSCE（把高維 kNN 的模糊隸屬度拉進 latent 距離）。架構、
decoder、Poisson NLL 跟前面所有 v2 版本一致，差別只有這三件事。

為什麼要有這一版：v2_ae_tanh_fsce 的筆記假設「FSCE 的推力可以補償 tanh 飽和
造成的訊號壓縮」，v2_dae 的筆記假設「加噪可以擋掉 latent 退化成密度計」。
這一版把兩個假設疊起來測——最好的情況是 DAE 逼出來的類別組成結構被 FSCE
排進一個有界、乾淨、填得滿的 (-1,1)² 盒子裡；最壞的情況是三種正則化互相
擠壓，可用資訊被削到只剩總量。這是要實測的。
...
def corrupt(x, p, mode="thinning", generator=None):
    ...
    p：破壞強度 ∈[0,1)。thinning 是每個 POI 被丟掉的機率，mask 是每一類
       整維被抹成 0 的機率。p=0 時直接回傳 x（等價於退化成 v2_ae_tanh_fsce）。
...
class MLPAE(nn.Module):
    """跟 v2_dae_fsce 的 MLPAE 唯一差異：latent 前多接一個 Tanh()，
    權重形狀跟 v2_ae_tanh/v2_ae_tanh_fsce 一致（checkpoint 可互換）。
