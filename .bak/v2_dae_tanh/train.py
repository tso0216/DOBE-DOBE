"""訓練 v2_dae_tanh 的 denoising MLP AutoEncoder（latent 被 tanh 夾住），
輸出每個 patch 的 latent 與 Poisson deviance。

跟 v2_dae 的 train.py 完全一樣，只是換了 VERSION/模型（encoder 多一個
Tanh()）。訓練迴圈裡餵給模型的是 corrupt() 破壞過的 count 向量，
Poisson NLL 仍然拿乾淨的原始 count 當目標；驗證與最後輸出 latent 的
推論階段一律不加噪——噪聲是正則化手段，不是資料本身的性質。

破壞是每個 step 重新抽的（generator 綁 SEED+epoch，可重現），同一個 patch
在不同 epoch 會看到不同的殘缺版本。

其餘超參數（latent 2 維、EPOCHS、lr 1e-3、切分 seed、NOISE_P/NOISE_MODE）
全部跟 v2_dae 對齊，這樣兩邊的差異只來自 tanh 這一個變因。
"""
