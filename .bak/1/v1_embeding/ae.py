"""v1_embeding：輸入從「每類一個 count channel」換成「每格一個類別 embedding」。

跟 v0 的差別只有輸入的長相，架構與 loss 完全一樣（in = out + 逐格 MSE）：

  v0            (10, 40, 40)   10 channel = 10 類的 log1p(count)
  v1_embeding   ( 8, 40, 40)   8 = embedding 維度，每格放「一個」POI 的類別向量
