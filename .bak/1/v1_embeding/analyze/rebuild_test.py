"""挑一個 patch 丟進訓練好的 v1_embeding AE，比較「進去」與「出來」。

三張圖：
  左   原始 POI（半徑 300m 的圓，一點一個，顏色 = 類別）
  中   實際餵給 AE 的輸入：每格只留一個隨機代表，畫在格子中心
  右   AE 重建：每格是一個 8 維向量，取它跟 10 個類別 embedding 的內積最大者
       當「這格 AE 覺得是什麼類」，向量長度當強度（長度 0 的格子看不見）

另外印出這個 patch 的 MSE、它在全體裡的百分位，以及兩個比 MSE 好懂的指標：
  佔用格命中率  取重建強度前 N 名的格子（N = 真實佔用格數），有幾成落在真的有東西的格子
  類別正確率    在真實佔用的格子上，重建的最近類別猜對的比例（亂猜 = 1/10）
"""
...
LATENTS = result("v1_embeding", "latents.npz")
CKPT = result("v1_embeding", "ae.pt")
OUT = result("v1_embeding", "rebuild_test.png")
...
def draw_recon(ax, score, inten, title):
    """重建：顏色 = 最近的類別，大小/透明度 ∝ 向量長度。"""
    x, y = cell_xy()
...
def main():
    ...
    print(f"佔用格命中率 {hit:.1%}（亂猜 {n_occ / (GRID * GRID):.1%}）  "
          f"類別正確率 {acc:.1%}（亂猜 {1 / N_CAT:.1%}）")

    print("\n逐類別：輸入格數 -> 重建判給這一類的格數（只算前 N 名的強格）")
    strong = np.zeros(GRID * GRID, dtype=bool)
    strong[top] = True
    strong = strong.reshape(GRID, GRID)
    pred_cat = score.argmax(0)
    for c, name in enumerate(CAT_ZH):
        print(f"  {name:<6}輸入 {int((true_cat == c)[occ].sum()):4d} 格 -> "
              f"重建 {int((pred_cat == c)[strong].sum()):4d} 格")
    ...
    draw_recon(c, score, inten, f"AE 後（MSE {loss:.6f}）")
    ...
    fig.suptitle(f"v1_embeding patch {n}（latent_dim={LATENT_DIM}）", fontsize=12)
    fig.tight_layout()
