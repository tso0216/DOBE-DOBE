"""畫出 v1_embeding 的 latent space，並檢查它到底編碼了什麼。

跟 v0 同一套指標，方便直接對照：用 kNN 回歸算「從 latent 能還原多少 POI 總數」
的 R²。v1 的輸入每格只留一個代表，密度資訊被大幅削掉，所以這個 R² 應該要比
v0 低；如果還是很高，代表 latent 仍然被「有多少格被佔滿」主導。
"""
...
OUT = result("v1_embeding", "latent_plot.png")
...
def main():
    ...
    fig.suptitle(f"v1_embeding latent space（{len(z)} 個 patch，latent_dim=2，"
                 f"類別 embedding + in = out + MSE）", fontsize=12)
    fig.tight_layout()
