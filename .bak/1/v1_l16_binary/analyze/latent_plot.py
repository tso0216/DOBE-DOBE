"""畫出 v1_l16_binary 的 latent space，並檢查它到底編碼了什麼。

v1_l16_binary 輸入是 binary presence（0/1），loss = BCE。
跟 v0_l32 結構完全一樣（latent 32 維），差別只有格值語意，
所以這裡的圖可以直接對比 v0_l32/analyze/latent_plot.py 的結果。

latent 是 32 維畫不出來，散點圖一律先用 PCA 投影到 2 維（PC1/PC2）；
kNN R² 與 robust 距離仍然用完整的 32 維算。
"""
...
LATENTS = result("v1_l16_binary", "latents.npz")
OUT = result("v1_l16_binary", "latent_plot.png")
...
def pca2(z):
    """把 latent 投影到前 2 個主成分，回傳 (投影後座標, 各自解釋的變異比例)。"""
    zc = z - z.mean(0)
...
def main():
    ...
    zoom(e, zp)
    scatter(e, zp, err, "放大：色=BCE 重建誤差", "BCE")
    ...
    style(f, f"全域：robust 距離離群點（距離用完整 {z.shape[1]} 維算）", *axes_labels)
    ...
    fig.suptitle(f"v1_l16_binary latent space（{len(z)} 個 patch，"
                 f"latent_dim={z.shape[1]}，binary presence + BCE，"
                 f"散點為 PCA 前 2 維）", fontsize=12)
    fig.tight_layout()
