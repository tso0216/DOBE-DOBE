"""比較 v1_masked AE 的重建 MSE 跟兩個沒有學習能力的 baseline。

三個數字都在同一個 embedding 空間、同一套 train/val 切分下算，而且都只算佔用格：
  1. AE       模型重建的 MSE
  2. zeros    永遠輸出全 0。佔用格上 ||x|| = 1，所以這個 baseline 恆等於
              1 / EMB_DIM = 0.125，每個 patch 都一樣
  3. mean     永遠輸出訓練集的平均矩陣（每格每維的平均 embedding）

AE 至少要贏 mean 才代表它真的有學到「這個 patch 長什麼樣」。
注意 embedding 是跟著模型一起學的，所以這裡的 MSE 尺度跟 v0 / v1_embeding
不能直接比大小，只能看「AE 相對 zeros / mean 贏多少」這種比例。
如果 AE 的絕對值掉到接近 0，那不是學得好，是 embedding 塌成常數了。
"""
...
def evaluate(data, idx, model, mu):
    """回傳這組 patch 的三種 per-patch MSE（都只算佔用格）。"""
    ae, zeros, mean = [], [], []
    with torch.no_grad():
        for i in range(0, len(idx), BATCH):
            g = data.render(idx[i:i + BATCH]).to(device)
            x, _, recon = model(g)
            ae.append(mse_loss(recon, x, g).cpu())
            zeros.append(mse_loss(torch.zeros_like(x), x, g).cpu())
            mean.append(mse_loss(mu.expand_as(x), x, g).cpu())
