"""畫出 v0_nb 的 latent space，並檢查它到底編碼了什麼。

跟 v0_poisson_nll 對照著看的重點只有一個：kNN R²（從 latent 能還原多少
POI 總數）有沒有變。NB 只是把 likelihood 換成能容忍 overdispersion 的版本，
並沒有碰「密度佔滿 latent」這件事，所以預期 R² 不動。
如果真的不動，就等於證實了噪聲模型不是瓶頸，該去處理 size factor。

色階裡的「重建誤差」是 NB deviance（見 ae.py），不是 MSE 也不是 Poisson deviance。
"""
...
LATENTS = result("v0_nb", "latents.npz")
OUT = result("v0_nb", "latent_plot.png")
...
def main():
    ...
    fig.suptitle(f"v0_nb latent space（{len(z)} 個 patch，latent_dim=2，"
                 f"raw count + 圓內 NB NLL）", fontsize=12)
    fig.tight_layout()
