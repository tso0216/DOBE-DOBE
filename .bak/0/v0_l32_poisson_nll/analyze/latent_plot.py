"""畫出 v0_l32_poisson_nll 的 latent space，並檢查它到底編碼了什麼。

跟 v0 對照著看的重點是 kNN 回歸的 R²（從 latent 能還原多少 POI 總數）：
Poisson NLL 本來就把「這個密度有多不尋常」納入 likelihood，理論上不必
像 v0_weight_mse 那樣人工壓低密度的份量。如果這裡的 R² 仍然跟 v0 一樣高，
代表換 loss 沒有把 latent 從密度手上搶回來，得回頭看是不是幾何主導。

色階裡的「重建誤差」是 Poisson deviance（見 ae.py），不是 MSE。
"""
...
OUT = result("v0_l32_poisson_nll", "latent_plot.png")
...
def main():
    ...
    fig.suptitle(f"v0_l32_poisson_nll latent space（{len(z)} 個 patch，latent_dim=32，"
                 f"raw count + 圓內 Poisson NLL）", fontsize=12)
    fig.tight_layout()
