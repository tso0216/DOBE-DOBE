...
跟 v0 版的差別：矩陣是 raw count（不套 log1p），誤差曲線畫的是圓內的
Poisson deviance。灑點只灑在圓內，所以加進去的量全部落在 loss 算得到的
區域，deviance 應該隨 k 單調上升——如果沒有，代表模型直接把 λ 拉高
吃掉了整個增量，那 latent 的移動就不能解釋成「過飽和」。
"""
...
OUT = result("v0_l32_poisson_nll", "make_outlier.png")
...
def run_cat(model, base, cat_id, rng):
    """對某一類跑完整條軌跡，回傳 (steps, REPEATS, 2) 的 z 與 deviance。"""
    dx0, dy0, cat0 = base
...
def main():
    ...
    e.set_title("重建誤差（圓內 deviance）", fontsize=10)
