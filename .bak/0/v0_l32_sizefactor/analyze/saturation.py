...
size factor 把密度從 latent 拿掉之後，兩者才分得開：
    z            = 形狀，跟總量無關
...
已知風險：
  * 「形狀」這兩個字要小心。在 2 維的 v0_sizefactor 上，latent_features.py
    量到組成根本沒進 latent（entropy R² = 0.014、十類佔比全部 < 0.11），
    latent 幾乎只編了「POI 離中心多遠」的徑向剖面（mean_r R² = 0.722）。
    那一版的「形狀鄰居」其實是「徑向剖面鄰居」。這一版開到 32 維就是要看
    組成會不會回來——**先跑 analyze/latent_features.py 看那張表**，再決定
    底下這個 z-score 能不能叫「同樣長相的地方」。
  * 鄰居的定義完全由模型決定。latent 若塌成一條線，鄰居就沒有意義，
    要先看 latent_features 的 PCA 解釋比例與散布。
  * k=50 在三萬個 patch 上約佔 0.2%，但 latent 邊緣的點鄰居會被拉得很遠，
    那裡的 z-score 不可信。所以圖上另外標出「鄰居平均距離」最大的那批。
  * patch 中心是規則格點（CENTER_STEP=50m），而窗是 1600m，相鄰 patch 重疊
    約 94%，kNN 很可能挑到地理上的鄰居而不是形狀上的同類。這會讓 z-score
    偏保守（自己跟自己比）。要根治得在取鄰居時排除地理距離太近的，還沒做。
  * 32 維版本多一層維度詛咒：高維下點對點的距離趨於均勻，「最近的 50 個」
    比 2 維時更接近「隨機的 50 個」，z-score 會再保守一些。判讀時看排序
    （誰最過飽和）比看絕對值可靠。
  * 散點圖畫不出 32 維，先 PCA 投影到前 2 維；kNN 與 z-score 都用完整 32 維
    算，只有那張圖是投影。PCA 解釋比例會印出來，太低的話那張圖只是示意。
"""
...
OUT = result("v0_l32_sizefactor", "saturation.png")
...
def main():
    ...
    dist, idx = cKDTree(zs).query(zs, k=KNN + 1, workers=-1)
    nb = idx[:, 1:]                      # 排除自己
    mu = y[nb].mean(1)
    sd = y[nb].std(1)
    resid = y - mu
    ...
    print(f"{len(z)} 個 patch，latent_dim={z.shape[1]}，k={KNN}")
    print(f"PCA 前兩軸解釋 {ratio[0]:.1%} + {ratio[1]:.1%} = {ratio.sum():.1%} "
          f"的變異（只有散點圖用投影，其餘都用完整維度）")
    ...
    sc = a.scatter(zp[:, 0], zp[:, 1], c=score, s=DOT, cmap="coolwarm",
    ...
    a.set_xlabel("PC1", fontsize=8)
    a.set_ylabel("PC2", fontsize=8)
    ...
    fig.suptitle("v0_l32_sizefactor 飽和度：latent 只管形狀，密度單獨比較\n"
                 f"（{len(z)} 個 patch，latent_dim={z.shape[1]}，"
                 f"k={KNN} 個形狀鄰居；散點為 PCA 前 2 維，"
                 f"解釋 {ratio.sum():.0%} 變異）", fontsize=12)
    fig.tight_layout()
