"""畫出 v0_sizefactor 的 latent space，並檢查它到底編碼了什麼。

這一版的驗收標準跟其他版本不一樣。其他版本是「希望 R²密度 低一點」，
這裡是「R²密度 應該要塌下來」——密度已經被 size factor 從 encoder 的輸入
裡除掉了，如果 latent 還能還原 log(POI 數)，代表正規化沒生效，是 bug。

所以多印兩個組成類的量，看密度讓出來的空間被什麼填滿：
  最大類佔比   單一類別獨大的程度（車站周邊 vs 商店街）
  組成熵       類別分布有多平均，越高越混合
如果這兩個的 R² 明顯比 v0_poisson_nll 高，就是這一版真正的收穫。

「過飽和」的判定改到 analyze/saturation.py，這裡的 robust 距離離群
只代表「形狀怪」，不再等於「密度異常」。
"""
...
from config.dataset import CAT_ZH, N_CAT, PATCHES, HALF_WIDTH, result  # noqa: E402

LATENTS = result("v0_sizefactor", "latents.npz")
OUT = result("v0_sizefactor", "latent_plot.png")
...
def patch_stats(p):
    """一次掃過 patches，回傳 (偏心度, 最大類佔比, 組成熵)。"""
    offs, dx, dy, cat = p["offsets"], p["dx"], p["dy"], p["cat"]
    n = len(offs) - 1
    ecc = np.zeros(n)
    top = np.zeros(n)
    ent = np.zeros(n)
    for i in range(n):
        s, e = offs[i], offs[i + 1]
        ecc[i] = np.hypot(dx[s:e].mean(), dy[s:e].mean()) / HALF_WIDTH
        frac = np.bincount(cat[s:e], minlength=N_CAT).astype(float)
        frac /= frac.sum()
        top[i] = frac.max()
        nz = frac[frac > 0]
...
def main():
    ...
    lat, lon = d["lat"], d["lon"]
    ecc, top, ent = patch_stats(np.load(PATCHES))

    print(f"latent 能還原多少（kNN R², k={KNN}）")
    for name, y in [("log(POI 數) ← 該塌掉", np.log(n_poi.astype(float))),
                    ("偏心度", ecc),
                    ("最大類佔比", top),
                    ("組成熵", ent),
    ...
    scatter(a, zp, np.log10(n_poi), "全域：色=POI 數（應該看不出結構）",
            "log10(POI 數)")
    zoom(b, zp)
    scatter(b, zp, top, f"放大 {ZOOM_PCT[0]}~{ZOOM_PCT[1]}%：色=最大類佔比",
            "max 類別佔比", cmap="plasma")
    zoom(c, zp)
    ...
    f.scatter(zp[out, 0], zp[out, 1], s=DOT * 4, c="#c0392b",
              ...
              label=f"形狀離群 (>{OUTLIER_PCT}%，n={out.sum()})")
    ...
    g.scatter(lon[out], lat[out], s=8, c="#c0392b",
              linewidths=0, alpha=0.85, rasterized=True, label="形狀離群 patch")
    g.legend(fontsize=8, markerscale=2, framealpha=0.9)
    g.set_aspect(1 / np.cos(np.deg2rad(float(lat.mean()))))
    g.set_title("形狀離群 patch 的地理位置", fontsize=10)
    g.set_xlabel("經度", fontsize=8)
    g.set_ylabel("緯度", fontsize=8)
    g.tick_params(labelsize=7)
    g.grid(alpha=0.15, linewidth=0.5)

    fig.suptitle(f"v0_sizefactor latent space（{len(z)} 個 patch，latent_dim=2，"
                 f"size factor 正規化 + 圓內 Poisson NLL）", fontsize=12)
    fig.tight_layout()
