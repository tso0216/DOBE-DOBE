"""畫出 v0_l16 的 latent space，並檢查它到底編碼了什麼。

v0_l16 是最單純的 in = out + MSE，latent 16 維，
沒有任何防止「latent 被密度吃掉」的設計。
所以這裡除了畫圖，還用 kNN 回歸算「從 latent 能還原多少 POI 總數」的 R²，
當成後面版本要打敗的對照數字。

latent 是 16 維畫不出來，散點圖一律先用 PCA 投影到 2 維（PC1/PC2）；
...
def main():
    ...
    style(f, "全域：robust 距離離群點（距離用完整 16 維算）", *axes_labels)
    ...
    fig.suptitle(f"v0_l16 latent space（{len(z)} 個 patch，"
