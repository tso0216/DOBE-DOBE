"""訓練 v0_weight_mse 的 in = out ConvAE，輸出每個 patch 的 latent 與重建誤差。

跟 v0 唯一的差別是 loss 用 ae.py 的加權 MSE（非零位置權重 POS_WEIGHT），
超參數刻意保持跟 v0 相同，這樣兩邊的 latent 才是可比較的。
存進 latents.npz 的 err 是加權後的數值，不能直接跟 v0 的 err 比大小。
...
OUT = result("v0_l32_weight_mse", "latents.npz")
CKPT = result("v0_l32_weight_mse", "ae.pt")
