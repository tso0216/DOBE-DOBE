"""畫出 v0_l32_weight_mse 的 latent space，並檢查它到底編碼了什麼。

這一版把非零位置的權重提高，就是為了不讓 latent 被「POI 總數」吃掉，
所以這裡的 kNN 回歸 R²（從 latent 能還原多少 POI 總數）要跟 v0 對照著看：
R² 明顯低於 v0 才代表加權真的把 latent 從密度手上搶回來了一些。
"""
...
LATENTS = result("v0_l32_weight_mse", "latents.npz")
OUT = result("v0_l32_weight_mse", "latent_plot.png")
...
def main():
    ...
    fig.suptitle(f"v0_l32_weight_mse latent space（{len(z)} 個 patch，latent_dim=32，"
                 f"in = out + MSE）", fontsize=12)
    fig.tight_layout()
