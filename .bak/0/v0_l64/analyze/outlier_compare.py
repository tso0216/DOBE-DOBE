"""比較 latent space 上的極端離群 patch 與中心（基準）patch。

取 robust 距離最大的 100 個當離群、最小的 100 個當基準，
用 --n 指定要看第幾名的離群 patch（1 = 最極端）。
...
OUT = result("v0_l64", "outlier_compare.png")
...
def main():
    ...
    ap.add_argument("--n", type=int, default=1,
                    help=f"要看第幾名的離群 patch（1~{TOP_K}，1 = 最極端）")
    n = ap.parse_args().n
    assert 1 <= n <= TOP_K, f"--n 要在 1~{TOP_K}"
    ...
    print(f"離群 #{n}: patch {oi}  robust 距離 {dist[oi]:.2f}  "
          f"POI {n_poi[oi]}  ({lat[oi]:.5f}, {lon[oi]:.5f})")
    print(f"基準 #{n}: patch {bi}  robust 距離 {dist[bi]:.2f}  "
          f"POI {n_poi[bi]}  ({lat[bi]:.5f}, {lon[bi]:.5f})")
    print(f"離群 100 個 POI 數中位數 {np.median(n_poi[out_idx]):.0f}，"
          f"基準 100 個 {np.median(n_poi[base_idx]):.0f}，"
    ...
    fig.suptitle(f"v0_l64 latent space：離群 #{n} vs 基準 patch"
