...
from model.other.v2_deep_ae.cfg import CKPT, LATENT_DIM, METRIC
...
def test_mse(model, data, idx, batch=256):
    """model：AE。data：Patches。idx：要評估的 patch 編號 tensor（例如 test split）。
    batch：每批大小。回傳這批 idx 的 (mse, mae, mape) 三個 float，mape 只算真實
    count > 0 的格子。
    """
    model.eval()
    diffs, counts = [], []
