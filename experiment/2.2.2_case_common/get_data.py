"""挑一個 common patch（與 latent 最近 k 鄰居的平均距離最小），窮舉所有「加入 1~B_MAX 個 POI」的組合，記錄每個預算下分數最高的組合、latent 位置與分數"""
import csv
import itertools
import os
import sys
...
PICK = "common"               # outlier：挑分數最大的；common：挑分數最小的
OBJ = "max"                   # min：把分數壓到最低；max：把分數推到最高
