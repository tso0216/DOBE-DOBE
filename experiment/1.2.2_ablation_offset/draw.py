"""繪製 get_data.py 輸出的六個 AE/DAE 變體，各 POI 類別加入不同數量後的 latent 平均偏移"""
import csv
import math
import os
...
def main():
    ...
    fig.suptitle("各 POI 類別加入不同數量後的 latent 平均偏移（ablation）")
    fig.tight_layout()

    out = os.path.join(out_dir, "1.2.2_ablation_offset.png")
