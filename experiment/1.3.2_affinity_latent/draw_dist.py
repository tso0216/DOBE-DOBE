...
UNIFORM = "UNIFORM"
UNIFORM_ZH = "均勻加（對照）"
NEIGHBOR = "latent 最近 8 鄰居"
OUT = "1.3.2_affinity_latent_dist.png"
BINS = 20
NCOLS = 4
...
def main():
    ...
    for ax, cat in zip(axes.flat, cats):
        ...
        pool = [v for amt in amounts for v in deltas[(cat, amt)]]
    ...
    for ax in axes.flat[:len(cats)]:
        ax.set_xlabel("與沒加時的分數差距 Δ（負值＝距離縮短）", fontsize=8)
