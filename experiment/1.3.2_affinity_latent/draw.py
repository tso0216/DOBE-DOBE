"""繪製 get_data.py 輸出的各 POI 類別加入不同數量後，與 latent 最近 k 鄰居的 latent 平均距離變化、改善比例與扣除總量效應後的淨效應"""
import csv
import math
import os
import sys
...
def main():
    ...
    axes[0].set_title(f"各 POI 類別加入後與 latent 最近 8 鄰居的平均距離變化"
                      f"（負值＝更典型；誤差棒為 95% CI，n={n_patch}）", fontsize=11)
    ...
    axes[2].set_title("扣除總量效應後的淨效應（負值＝比同總量均勻加更典型，逐 patch 配對相減）",
                      fontsize=11)
    axes[2].legend(fontsize=9)

    fig.tight_layout()
    out = os.path.join(HERE, "1.3.2_affinity_latent.png")
