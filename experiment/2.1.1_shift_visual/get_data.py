"""計算六個 AE/DAE 變體，在各 POI 類別加入 POI 後每個 patch 的 latent 位移軌跡"""
import csv
import os
import sys
...
def shift_path(encode_fn, x, cat_idx, amounts):
    """encode_fn：(B, N_CAT) tensor -> (B, 2) tensor 的 encode 函式。x：(B, N_CAT) count tensor。
    cat_idx：要加 POI 的類別索引。amounts：由小到大的加入量清單。
...
def main():
    ...
    with open(out_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["model", "category", "sample_id", "step", "amount", "z1", "z2"])
        for name, label in MODELS:
            model = load_model(os.path.join(MODEL_DIR, name))
            for cat_idx, category in enumerate(CATEGORIES):
                path = shift_path(lambda x, model=model: encode(model, x),
                                   x_test, cat_idx, ADD_AMOUNT)
                ...
                for sample_id in range(path.shape[0]):
                    for step, amount in enumerate(cum_amounts):
                        ...
                        writer.writerow([label, category, sample_id, step, amount, z1, z2])
    print(f"已存 {out_path}")
