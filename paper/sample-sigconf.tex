...
The input vector $x_A$ is passed directly through the trained Encoder (without corruption) to obtain its representation $z_A$ in the latent space. We then calculate $S_A$, defined as the leave-one-out kNN average latent distance against the test set. A higher score signifies that the region's functional composition is an outlier relative to its peers, whereas a lower score indicates a highly typical, well-balanced structure.
...
We monitored the topological integrity during training by calculating the neighbor preservation rate, defined as the overlap of the top $k=10$ neighbors between the high-dimensional space and the 2D latent space. Figure \ref{fig:neighbor_preservation} confirms that the preservation rate reliably climbs as training progresses. This proves FSCE successfully forces the model to respect real-world functional relationships, guaranteeing that patches situated close together in the latent space truly reflect comparable urban capacities in reality.
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{figure/neighbors.png}
    \caption{Neighbor Preservation across Training Stages. The steady increase proves FSCE anchors the low-dimensional embeddings to high-dimensional contextual realities.}
...
\end{figure}
To validate the practical utility of the framework, we mined real-world cases from the dataset to observe whether simulated POI additions correctly reflect reasonable or unreasonable urban planning choices. 

Figure \ref{fig:case_study} illustrates both a common case and an outlier case. The outlier case represents a location initially lacking complementary infrastructure (starting score: 2.60). By systematically adding specific supportive POIs, the framework pulls this imbalanced patch back into a typical, functionally harmonious zone (ending score: 0.39). This proves the model can accurately identify when an area requires complementary amenities to achieve structural balance. 

Conversely, the common case shows a standard, highly typical area (starting score: 0.25) receiving heavy POI additions. The framework identifies that these unchecked additions push the area outward into the latent periphery, transitioning it into a highly unique or potentially over-concentrated hub (ending score: 4.04). This directly validates the core premise of our study: rather than relying on absolute density caps, the system effectively discerns whether an area's underlying context makes a new POI addition reasonable or structurally disruptive.


\section{Conclusion}

This study introduced a peer-relative spatial decision support system designed to evaluate urban structural balance and assess the suitability of new POI additions. By integrating TF-IDF preprocessing with a Denoising Autoencoder and FSCE topological constraints, our framework successfully transitions locational evaluation from absolute density estimation to a contextual harmony assessment. Experimental findings confirm that TF-IDF effectively suppresses globally dominant categories to clarify functional boundaries, while the joint optimization strategy successfully anchors low-dimensional representations to realistic urban topologies. The real-world case studies further demonstrate the system's ability to identify whether introducing new facilities will stabilize an imbalanced area or structurally overload a typical one.
