...
\begin{figure}[htbp]
    ...
    \includegraphics[width=.9\linewidth]{figure/fig1.png}
...
This experiment examines how much reconstruction capability is traded off when topological constraints are introduced. Table \ref{tab:main_results} compares our proposed architecture against standard baselines, including the plain DAE. The plain DAE achieves the best reconstruction on all three metrics, and the full model is slightly worse than it (MAE 0.6487 vs.\ 0.6442, MSE 1.3356 vs.\ 1.3302, WAPE 0.3720 vs.\ 0.3651, i.e., relative increases of roughly 0.7\%, 0.4\%, and 1.9\%). This gap is the cost of adding TF-IDF-based peer construction and FSCE regularization, whose purpose is to preserve peer relationships in the latent space rather than to further improve reconstruction; their effect is examined separately in Section~\ref{sec:qualitative}. At the same time, the full model still outperforms PCA, AE, and VAE on all three metrics, indicating that it retains the POI composition well enough to serve as a basis for peer-relative scoring.
...
\midrule
Ours (DAE+FSCE+TFIDF) & 0.6487 & 1.3356 & 0.3720 \\
\bottomrule
\end{tabular}
\end{table}
...
To evaluate the contribution of each module, we systematically ablate the TF-IDF preprocessing, FSCE graph loss, and the Denoising mechanism. Table \ref{tab:ablation_results} groups the purely AE-based configurations and the DAE-based configurations. As also reported in Table \ref{tab:main_results}, the plain DAE achieves the lowest reconstruction errors, and adding FSCE and TF-IDF increases them slightly. We therefore do not claim that these modules improve reconstruction; they are included to shape the peer structure of the latent space, which we examine in the subsequent qualitative analysis. Across all three configurations, the DAE-based models achieve lower MAE, MSE, and WAPE than their AE counterparts, suggesting that denoising helps maintain reconstruction quality when the topological constraints are enforced.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{figure/fig5.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{figure/fig7.png}
